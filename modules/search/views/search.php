...
<section id="mpart">
	<div class="container">
		...
		<div class="row mt40">
			...
			<div class="col-md-9">
				<h3 class="section-title">You search: <?=(isset($_GET['q'])) ? $_GET['q'] : null?></h3>
				<br/>
				...
				if(isset($_GET['q']))
				{
					...
					// 	$product_search2 = array();
					// 	$queries = explode(' ', $q);
					// 	foreach($queries as $qs){ $product_search2[] = "LOCATE('$qs', product.product_title)"; }
					// 	$product_search2 = implode(" AND ", $product_search2);
					// 	//$sql = "SELECT *, $product_search2 FROM product WHERE $product_search2 > 0 ORDER BY $product_search2";
						
					// 	//$sql = "SELECT * FROM `product` INNER JOIN `term_relation` WHERE product.product_id = term_relation.product_id AND term_relation.term_id = $cat";
					// 	//$sql = "SELECT *, $product_search2 FROM product INNER JOIN `term_relation` WHERE $product_search2 > 0 AND product.product_id = term_relation.product_id AND term_relation.term_id = $cat ORDER BY $product_search2";
					// 	$sql = "SELECT *, $product_search2 FROM product INNER JOIN `term_relation` WHERE $product_search2 > 0 AND product.product_id = term_relation.product_id AND term_relation.term_id = $cat ORDER BY $product_search2";
					// }
					...
					if($products)
					{
						if(count($products)==1)
						{
							...
							redirect(base_url($prod_row->product_name.'/?q='.urlencode($_GET['q'])));
						}
						else
						{
					...
					{
						echo '<p style="border:1px solid green;padding:5px;width:100%;font-size:16px;">No available product now!</p>';
					}
				}
				else
				{
